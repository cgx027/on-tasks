...
function installOsJobFactory(
    ...
    // InstallOsJob.prototype._unsubscribeRequestProfile = function _unsubscribeRequestProfile() {
    //     if (self.profileSubscription) {
    //         _.pull(self.subscriptions, self.profileSubscription);
    //         console.log('-----------------------unsubscription called')
    //         return self.profileSubscription.dispose();
    //     }
    // };
    ...
    InstallOsJob.prototype._run = function() {
        ...
        return Promise.resolve().then(function() {
            ...
            console.log('----------------------Ted, in subscribe ', self.taskId)

            self._subscribeTaskNotification(self.taskId, function(data) {
                assert.object(data);

                console.log('--------Ted, in subscribe ', taskId, data)
                ...
                if(
                    ...
                    && data.data.progress === 'osInstalled'
                ) {
                    // installed = true;
                    self._unsubscribeRequestProfile();
                    logger.info('OS install task ongoing', data);
                }
            });
        }).catch(function(err) {
