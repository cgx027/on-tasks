...
function factory(
    ...
    function Task(definition, taskOverrides, context) {
        ...
        } else if(configuration.get('on-static')) {
            var onStaticConfig = configuration.get('on-static');

            var onStaticAddr = onStaticConfig.southbondAddress ;
            var onStaticPort = onStaticConfig.southbondPort ;

            assert.string(onStaticAddr, "southbondAddress") ;
            assert.number(onStaticPort, "southbondPort") ;

            fileServerUri = 'http://' + onStaticAddr + ':' + onStaticPort;
        } else {
